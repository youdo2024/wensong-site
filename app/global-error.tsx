...
export default function GlobalError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  ...
  return (
    <html lang="zh-Hant-TW">
      <body style={{ margin: 0, background: "#EFE3C4", color: "#3A3226",
        fontFamily: '"Noto Serif TC","Source Han Serif TC",serif' }}>
        <div style={{ maxWidth: 520, margin: "0 auto", padding: "84px 20px" }}>
          <div style={{ border: "2px solid #3A3226", background: "#FFFDF6", textAlign: "center" }}>
            <div style={{ height: 8, background: "#3A3226" }} />
            <div style={{ padding: "44px 24px" }}>
              <span style={{ fontSize: 12, letterSpacing: ".28em", color: "#7C7060" }}>暫時無法顯示</span>
              <h1 style={{ fontSize: 22, fontWeight: 900, letterSpacing: ".12em", margin: "14px 0 0" }}>
                問爽的遇到一點狀況
              </h1>
              <p style={{ color: "#7C7060", fontSize: 14.5, lineHeight: 2, marginTop: 12 }}>
                是我們這邊的問題。先按「重新載入」，通常過一下就恢復了。
              </p>
              <p style={{ color: "#7C7060", fontSize: 14.5, lineHeight: 2 }}>
                如果你剛剛在結帳或贊助，<b style={{ color: "#3A3226" }}>錢還沒有被扣</b>。
              </p>
              <p style={{ marginTop: 24 }}>
                <button
                  onClick={() => reset()}
                  style={{ font: "inherit", fontSize: 14, letterSpacing: ".1em", padding: "11px 26px",
                    border: "2px solid #3A3226", background: "#3A3226", color: "#EFE3C4", cursor: "pointer" }}
                >
                  重新載入
                </button>
              </p>
              {error?.digest && (
                <p style={{ marginTop: 18, fontSize: 12, color: "#7C7060", letterSpacing: ".08em" }}>
